...
class ComfyUIClient:
    ...
    async def execute_workflow(self, workflow: ComfyWorkflow, output_dir: str = "outputs") -> Dict[str, Any]:
        wf_to_run = await self.load_and_prepare_workflow(
            workflow.workflow_json_path,
            workflow._replacements
        )

        prompt_id = await self.queue_prompt(wf_to_run)
        if not prompt_id:
            return {"error": "无法提交工作流到队列"}

        completed = await self.wait_for_prompt_completion(prompt_id)
        if not completed:
            return {"error": "工作流执行失败或超时"}

        history = await self.get_history(prompt_id)
        if not history:
            return {"error": "无法获取执行历史记录"}
    ...
    async def _get_outputs_for_node(
        self,
                        ...
                        try:
                            ...
                            if isinstance(message_data, str):
                                ...
                                if message.get("type") == "progress":
                                    ...
                                    print(
                                        f"  - 进度更新: 节点 {data.get('node', 'N/A')} "
                                        f"- 步数 {data.get('value', 0)}/{data.get('max', 1)}"
                                    )

                                if (
                                    message.get("type") == "execution_success"
    ...
    async def _download_file(self, file_data: Dict[str, str], target_dir: str) -> Optional[str]:
        ...
        output_sub_dir = os.path.join(target_dir, file_type)
    ...
    async def upload_file(
        ...
        try:
            with open(file_path, "rb") as f:
                files = {
                    "image": (filename, f.read(), mime_type)
                }
    ...
    @classmethod
    async def load_and_prepare_workflow(
        cls,
        workflow_path: str,
    ...
    async def queue_prompt(self, prepared_workflow: Dict[str, Any]) -> Optional[str]:
        print("正在将工作流提交到队列...")
        ...
        try:
            ...
            print("解析服务器响应时发生JSON错误。这通常意味着服务器返回了一个错误页面而不是有效的JSON。")
    ...
    async def view_tasks(self) -> Dict[str, List[Dict]]:
        try:
            ...
            completed_tasks = []
            running_and_queued_ids = {
                t["prompt_id"] for t in running_tasks
            } | {
                t["prompt_id"] for t in queued_tasks
            }

            for item in sortable_history:
                prompt_id = item["prompt_id"]

                if prompt_id in running_and_queued_ids:
                    continue
